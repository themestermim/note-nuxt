    ...
    >
        ...
        <div class="container flex flex-nowrap gap-2 px-4 pt-3 pb-2 h-full flex-system *:transition-all">
            <div class="w-auto flex-1 flex flex-col gap-0 inputs-col *:transition-all">
                <input type="text" v-model="note.title" id="title" name="title"
                       class="focus:outline-none text-sm font-medium pr-2 text-gray-600 input max-h-0 overflow-hidden"/>
                <textarea name="description" v-model="note.description" id="description"
                          class="transition-all h-11 text-base w-full font-normal placeholder:text-gray-400 border border-gray-200 rounded-lg resize-none py-1 px-2 focus:outline-none textarea"
                          placeholder="یادداشت خود را بنویسید ..."></textarea>
            </div>
            <div class="w-20 button-col">
                <button type="button" role="button" @click="addNote" :disabled="note.description === ''"
                        class="transition-all h-11 w-full rounded-lg flex items-center justify-center px-2 gap-2.5 text-sm bg-brown-500 text-white disabled:opacity-60 disabled:pointer-events-none disabled:select-none">
                    <span>ارسال</span>
                </button>
            </div>
        </div>
...
watch(() => noteStore.noteList.length, (len) => {
        note.title = `تسک ${len + 1}`
    },{ immediate: true })
</script>